...
class Approach1(Simulation):
    ...
    def fitness(self):
        ...
        avg_waiting_time = -self._collect_waiting_times()/self._n_cars_generated
        harmonice_mean = 2 * avg_queue_length * \
            avg_waiting_time/(avg_queue_length + avg_waiting_time)
        fitness = harmonice_mean
        print(f'#{self._genome_id}', fitness, "avg_waiting_time=",
              avg_waiting_time, "avg_queue_length=", avg_queue_length)
        return fitness
...
class Approach2(Simulation):
    ...
    def fitness(self):
        fitness = 0
        squared_list = [x**2 for x in self._collect_waiting_times()]
        ...
        fitness -= root_mean_squared_waiting_time
    ...
    def _collect_waiting_times(self):
        total_waiting = []
        for car_id, wait_time in self._waiting_times.items():
            total_waiting.append(wait_time)
    ...
    def _get_state(self):
        ...
        state = np.zeros(self._num_states)
        state_counter = np.zeros(self._num_states)
        car_list = traci.vehicle.getIDList()
        ...
        for car_id in car_list:
            ...
            if valid_car:
                state[car_position] += waiting_time
                state_counter[car_position] += 1

        for i in range(self._num_states):
            if state_counter[i] == 0:
                state[i] = 0
            else:
                state[i] /= state_counter[i]
...
class Approach3(Simulation): 
    ...
    def run(self, net) -> float:
        ...
        while curr_step < self._max_steps:
            ...
            current_state,new_car_ids = self._get_state()
...
class TimeBasedTrafficLigthSystem(Simulation):
    ...
    def fitness(self):
        ...
        fitness = 0
        squared_list = [x**2 for x in self._collect_waiting_times_list()]
        sum_squared = sum(squared_list)
        mean_squared = sum_squared/self._n_cars_generated
        root_mean_squared_waiting_time = mean_squared**0.5
    ...
    def _collect_waiting_times_list(self):
        total_waiting = []
        for car_id, wait_time in self._waiting_times.items():
            total_waiting.append(wait_time)
